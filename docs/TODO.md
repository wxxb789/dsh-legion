...
**Durable mutation stays unavailable.** The declared DSH 0.1.2-alpha.2 minimum and 0.1.2-alpha.3 latest-tested generations publish neither the atomic run coordination Host service nor a required-event admission seam for out-of-repository `legion/*` events. Alpha.3's `ignorable` envelope marker cannot represent Legion's state-changing facts. Session flush and `sessionProjections` exist, but neither closes those two independent blockers. Release evidence may prove structural contracts, pure replay, deterministic capability diagnostics, and unchanged ephemeral behavior; it must not claim production durable mutation. See the [0.1.2-alpha.3 audit](notes/dsh-0.1.2-alpha.3-upgrade.md).
...
- `devDependencies` target DSH 0.1.2-alpha.3, while the compatibility contract tests 0.1.2-alpha.2 as the minimum and alpha.3 as latest-tested. Main CI derives and packs the latest-tested source once, then installs its manifest-indexed tarballs without resolving DSH through npm. The scheduled `peer-range` canary and tag-release minimum/latest matrices retain public-registry installs because source compatibility does not prove distribution availability. `scripts/verify-packed-delegation.mjs` resolves every channel from `contracts/compatibility.json`, so workflow version literals cannot drift.
