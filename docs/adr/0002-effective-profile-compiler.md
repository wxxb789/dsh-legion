# ADR 0002: Compile one EffectiveProfile catalog before registration
...
Version 0.1 validated profile configuration in several places: plugin activation, provider lifecycle filtering, tool registration, prompt rendering, and the final start edge. Each location reconstructed part of the same policy. This was small, but it made future model metadata, route planning, explain diagnostics, and structured result contracts likely to drift.
...
- all detached EffectiveProfiles;
- the currently active profile map;
- stable typed diagnostics;
- an active default profile when available;
...
Profiles may select a versioned result contract:
...
- Profile order is canonicalized rather than preserving authored map insertion order.
- Structured profiles cannot default to continuable background execution.
