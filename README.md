...
This package does not ship DSH persistence, projection, atomic coordination, global admission, or child-receipt Host services. Published DSH 0.1.0-rc.6 lacks the projection and coordination services required for production durable mutation. Legion also ships no unified durable Strategy activation adapter yet, so a model-facing `execution: { durability: 'journal' }` request fails closed with `LEGION_DURABLE_EXECUTION_ADAPTER_UNAVAILABLE` on every Host, regardless of capability. Enabling durable runs there produces stable capability diagnostics and fails closed before mutation; pure contracts, validation, replay, and inspection remain usable. See [Durable Strategy Runs](docs/durable-runs.md) and [Journal Contract v1](docs/journal-contract-v1.md).
