...
- **The client bundle now compiles against the Host's published client contracts.** The hand-written `src/client/dsh-client.d.ts` mirror is gone; React, runtime, slot, locale, settings, and UI-primitives declarations now come from the assessed DSH line, and the card props are composed through `PropsRuntime`, `PropsLocale`, and `InjectFace`. An upstream client-surface change therefore fails `typecheck` instead of the page. `@deepseek-ai/dsh-client-ui-renderer` remains deliberately absent because it is boot-once shell machinery, while schema-form and React-web remain absent because neither is a source package. The loader-protocol suite now names the exact `web` platform and `./client` export guarantees separately.
