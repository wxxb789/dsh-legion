...
The committed pnpm 11 lockfile is enforced with `--frozen-lockfile`. Required CI covers Windows at exact Node 22.19.0, Ubuntu 24.04 at exact Node 24.19.0, and isolated packed consumers at the committed minimum and latest-tested compatible DSH generations. Two isolated `git archive HEAD` build/pack rounds must produce the same tarball digest, and that verified artifact is the one released. A scheduled rolling canary resolves the full peer range; release gates remain pinned to committed minimum/latest-tested generations and an exact parsed DSH package closure. The packed E2E installs the tarball into a clean consumer and executes one real, credential-free DSH child through the official in-process provider and a scripted LLM.
