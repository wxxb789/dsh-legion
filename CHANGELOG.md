...
- Tool presentation is documented as inherited and pinned as inherited. Whether a model sees every schema (`native`), only `run_code` plus a generated SDK (`code` — the Web client labels it **PTC mode**), or both is Host-owned: the official `@deepseek-ai/dsh-agent-tool-presentation` row decides it on a preset's standing scope, and Legion declares nothing, so it always runs the current official implementation with no version to pin. Delegated children inherit the same mode because `dsh-agent-presets` re-parents a child's scope onto that standing scope. `tests/tool-presentation.spec.ts` now fails if Legion ever declares a presentation, injects `codeRuntime`, hardcodes the reserved transport name, grows a presentation key of its own, or ships a preset that pins a mode — and it carries a negative control, so a scan that could never fire cannot pass as a gate. The shipped preset template carries the official row commented out with the reason it is off by default, and `docs/notes/code-mode-inheritance.md` records the evidence, including that profile `toolFilter` keeps its meaning under Code Mode: the SDK binding table is built from the calling agent's visible set, so the `review` profile's deny of `write`/`edit` holds in both presentations.
