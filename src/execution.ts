  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_resolve, reject) => {
        timer = setTimeout(() => reject(new Error('subagent disposal exceeded grace period')), 1000)
      }),
    ])
  } finally {
...
async function settleRun(
  ...
  if (result === undefined || result.stopReason !== 'completed') {
    throw new Error(`profile "${plan.profile}" ended with ${result?.stopReason ?? 'no result'}`)
  }
  return result
}
...
async function executeOne(
  ...
  return artifactValue(primitive.output, await settleRun(run, plan, signal))
}
...
export async function executeStrategyPlan(
  ...
  if (plan.primitives.some(primitive => primitive.kind === 'dsh-goal')) {
    return deepFreeze({
      kind: 'failed',
      ...
      failure: {
        stage: 'plan',
        member: 'goal',
        code: 'EXECUTION_CLASS_UNSUPPORTED',
        message: 'goal primitives require the future DSH goal execution adapter',
      },
    })
  }
  ...
  try {
    for (const primitive of plan.primitives) {
      ...
      if (primitive.kind === 'dsh-subagent-fanout') {
        const fanout = await executeFanout(ctx, catalog, primitive, prompt, parent, deadline.signal)
        ...
        try {
          const value = await executeOne(ctx, catalog, primitive, prompt, parent, deadline.signal)
    ...
    return deepFreeze(failures.length === 0
      ? { kind: 'completed', planDigest: plan.planDigest, artifacts: list, final }
      : { kind: 'degraded', planDigest: plan.planDigest, artifacts: list, final, failures })
  } catch (error: unknown) {
    const list = [...artifacts.values()]
