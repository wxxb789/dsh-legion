...
- **A version-coupled live Run Receipt companion.** The root installs the exact same-version `dsh-legion-receipts` package and mounts its separate Host/Client row beside the service-free Settings row. Official DSH Typert/Gateway carries complete Session baselines and replacements while the parent Session and companion instance remain live; Session disposal, companion reload, or Host restart starts empty. Unprovable remote dimensions stay unavailable and known aggregates may be partial, while headless or missing-companion execution keeps the bounded terminal summary.
...
- **The bundled preset now defaults to native presentation so its Review Specialist is genuinely read-only.** The Specialist uses an explicit `read`/`glob`/`grep` allowlist; it omits the conditionally registered `read_image` tool and every shell, mutation, Job, and delegation capability. PTC remains an explicit trusted-deployment option, but DSH's worker-thread code runtime documents bash-equivalent Node API authority and `run_code` cannot be filtered, so a PTC child is not a read-only security boundary even when its generated SDK excludes write tools.
- Optional PTC acceleration still reports a missing TypeScript runtime as an install instruction rather than leaving it to be inferred. When `ctx.codeRuntime` is absent, Legion logs the package and Host composition row that would enable PTC, then keeps working in the bundled preset's native presentation. The probe remains one optional `ctx.get?.('codeRuntime')`, never an `inject` dependency.
- The append-to-your-preset fragment carries no presentation row, because one composition selects one presentation and a second declaration is refused rather than merged. `tests/tool-presentation.spec.ts` pins the bundled native choice and the fragment's neutrality. DSH still builds PTC SDK bindings from the calling Agent's filtered visible tools, but `docs/notes/code-mode-inheritance.md` now records the separate code-runtime trust boundary: filtered SDK bindings do not remove ambient Node API authority from `run_code`.
...
- Projection state version 7 invalidates version-6 checkpoints after terminal first-wins became a projection invariant, forcing old last-write-wins cache rows to refold from the Session journal.
- **The settings card no longer comes and goes with sessions.** Legion's `cordis.patch.yml` was empty, so the package was mountable only from an Agent Preset — and both halves of a settings surface are process-wide. A namespace registration is an effect on the registering fiber, so `legion` was served exactly while a session using that preset was alive: the card appeared mid-session, vanished when the last one ended, was absent after a restart, and values already saved stayed in the user document with no way left to edit them. The Web half failed for a second, independent reason: the client module registry composes its table from the Host loader entries, and a preset subtree is plugged directly rather than created as a loader entry, so the card bundle was never discovered and never served to the page. The bundle patch now installs a `legion-settings` Host row that owns only the namespace/card and a separate `legion-receipts` Host row for the observation companion; neither publishes the model-facing delegation tool, because `tools` and `system-prompt` are layered registries and a Host row publishing that surface would hand it to every agent in the process. See [ADR 0023](docs/adr/0023-host-plane-settings-row.md).
...
- **The assessed DSH window is now 0.1.2-alpha.2 through 0.1.2-alpha.4.** Legion compiles against alpha.4 and preserves alpha.2 through one narrow Run Receipt read-boundary fallback. Alpha.4 removes `Session.events` and `SessionHeader.seedLength`, adds the official `snapshotEvents()`/`eventAt()` reads plus `Session.inheritedEventCount`, and brands event positions separately from log offsets. Legion now uses those official APIs, while its fallback reads alpha.2's immutable snapshot and inherited cut without copying the Session runtime. The Subagent `start` and `startContinuable` entry points remain compatible; removed continuable messaging/reporting/setup methods and `dsh-tool-subagent-report` are unused. The official dependency closure remains 74 packages, and `docs/notes/dsh-0.1.2-alpha.4-upgrade.md` records the source audit, reuse decisions, and rollback boundary. Durable mutation remains fail-closed because alpha.4 still supplies neither Host Run Coordination nor required external Session-event admission.
- **The assessed DSH window is now 0.1.2-alpha.2 through 0.1.2-alpha.3.** Legion compiles against alpha.3 and keeps alpha.2 as the minimum because alpha.2 moved `JsonValue`, `snapshotJsonValue`, and `deepFreeze` to the official duplicate-install-safe `@deepseek-ai/dsh-util-values` package and replaced the exported prompt-order table with `SystemPrompt.getSectionOrder()`. The Run Receipt client also uses the official Typert `RemoteError` and Gateway `isRemoteFailure()` ABI so an unavailable or malformed opening frame survives reconnecting-stream normalization. Cordis, Schemastery, and the Cordis loader/include tooling also align with the versions vendored by alpha.3. The compatibility contract records the complete 74-package DSH closure, source CI packs latest-tested independently of the minimum lane, direct test compositions mount the official projection registry, and `docs/notes/dsh-0.1.2-alpha.3-upgrade.md` records the source audit and rollback boundary. Journal mutation remains fail-closed because alpha.3 still supplies neither atomic Run Coordination nor a required external Session-event admission seam.
- **Specialist, Cohort, and Cohort Run are now the current public vocabulary everywhere.** Config v3, package-root schemas/brands/helpers/types, ACP helpers, Settings copy, tool guidance/results, examples, presets, and current documentation use canonical names. Published 1.x aliases remain explicit `@deprecated` adapters with unchanged behavior and removal no earlier than 2.0.0; no-target 1.x config materialization/export remains v2, and versioned Explain/result fields plus the `team-run-` identity prefix remain unchanged.
- **The assessed DSH line is now 0.1.2-alpha.1.** Legion follows the client Runtime split through `@deepseek-ai/dsh-client-store` and the feature-owned client contracts, selects the official native presentation for its read-only Review Specialist, uses the Host `ToolCallId` brand, derives prompt placement from `FIRST_PARTY_SECTION_ORDER`, and preflights the new `SubagentCapabilities.agentOptions` flag before child start. Provider snapshot v1 remains readable and defaults the newly introduced authority to `false`. CI compatibility channels resolve from `contracts/compatibility.json` instead of copying version literals. Because this Host line refuses persisted event types outside its generated vocabulary and exposes no plugin registration seam, full Run Receipt facts use the separate process-local `dsh-legion-receipts` companion with no custom Session event or storage; journal Strategy mutation remains fail-closed. The complete source audit is in `docs/notes/dsh-0.1.2-alpha.1-upgrade.md`.
- **Main CI verifies the assessed DSH source without requiring its npm publication.** One job derives the DSH tag from `contracts/compatibility.json`, builds and packs that source once, and every quality matrix installs the resulting tarballs through manifest-derived temporary dependencies. The public-registry canary and release gates remain separate, so source compatibility cannot silently stand in for distribution availability.
- **Install registry selection is explicit by environment.** `.npmrc` selects the Tencent mirror for constrained local development, while GitHub Actions sets `DSH_REGISTRY` to public npm. Packed/preflight scripts resolve the environment override first and the project setting second; `publishConfig.registry` independently keeps release publication on public npm.
...
- A settings-card write naming a `defaultSpecialist` no row defines is accepted and persisted because the namespace owner validates only catalog-independent facts; each delegation row judges its own Specialist catalog, keeps its last published generation when the selection is unavailable, and reports the rejection. Stored 1.x `defaultProfile` values still load with a deprecation diagnostic, while the next edited save atomically removes that key and writes only `defaultSpecialist`.
