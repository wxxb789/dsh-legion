...
The package requires Node.js `^22.19.0 || >=24.0.0` and DSH peers in `>=0.1.1-rc.1 <0.2.0`. Legion tracks the current Host release line: the peer range, the declared minimum, the latest tested version, and the assessed-version list in [`contracts/compatibility.json`](contracts/compatibility.json) advance together, and none of them advances alone. The range is anchored on a `0.1.1` prerelease comparator because semver admits a prerelease only against a comparator sharing its `major.minor.patch`, so every future `0.1.N-rc.x` Host line enters the range only once it has been assessed. CI covers Windows, Ubuntu, packed DSH consumers, public contracts, protocol benchmarks, and reproducible packages.
