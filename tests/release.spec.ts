...
describe('reproducible CI and release contracts', () => {
  ...
  it('resolves one exact DSH generation before installing every packed consumer dependency', () => {
    ...
    expect(consumer).toContain('configVersion: 3')
    expect(consumer).toContain('defaultSpecialist:')
    expect(consumer).toContain('specialists:')
    expect(consumer).toContain('cohorts:')
    expect(consumer).not.toMatch(/\b(?:defaultProfile|profiles|teams):/u)
    expect(script).toContain("join(projectRoot, 'tests', 'fixtures', 'packed-legacy-consumer.ts')")
