...
- the configured Specialist and subagent backend;
...
Legion does not widen DeepSeek Harness permissions. It selects among deployment-owned Specialists and delegates through the existing DSH subagent runtime. Sandbox, approval, credentials, product CLI authentication, and provider registration remain Host responsibilities.

A Specialist is trusted configuration. It can select a model backend and control a child's visible tools, but a tool filter is not a security boundary by itself. Do not install untrusted Cordis packages or user presets: they execute with the authority of the DSH process.
