...
Shipping that half costs three hand-maintained couplings, accepted deliberately because nothing else about the seam is speculative: DSH's client bundle preset is unpublished, so `tsdown.client.config.ts` mirrors its artifact format; the `settings.plugin.item` slot declaration is unpublished, so the card does not consume it; and the DSH client packages are published only on an older line than the Host, so the surface Legion uses is declared locally instead of imported. None of the three can fail the build — each fails the card at load time — so `tests/client-bundle.spec.ts` executes the built artifact under the loader's own protocol and asserts the id, the wrapper, and every requested specifier. The upstream publications that would retire all three are recorded in the roadmap.
