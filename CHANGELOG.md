...
- **Install registry selection is centralized.** `.npmrc` selects the Tencent npm mirror for development and CI installs; workflows no longer override it with a disabled endpoint, and packed/preflight scripts resolve that same project setting unless `DSH_REGISTRY` is explicitly supplied. `publishConfig.registry` keeps release publication targeted at public npm.
